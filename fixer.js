(() => {
    // --- Configuration ---
    const CONFIG = {
        PROBE_TIMEOUT: 2500,        // [FASTER] Give up on slow servers faster (was 5000)
        MAX_ATTEMPTS: 30,           // Check more candidates since we are faster
        MAX_SERVER_CAP: 20,         
        MAX_CONCURRENT_PROBES: 12,  // [FASTER] Increased parallel probes (was 4)
        CACHE_LIMIT: 2000,
        FAILED_CACHE_LIMIT: 5000,
        DEBUG: true,
        STORAGE_KEY_CACHE: 'bato_fixer_cache_v6', 
        STORAGE_KEY_CONTEXT: 'bato_fixer_context_v6'
    };
    ...
    async function processQueue() {
        ...
        while (processingQueue.high.length > 0 || processingQueue.low.length > 0) {
            const batch = [];
            const fill = (q) => {
                while (batch.length < 5 && q.length > 0) { // Larger processing batch
                    const img = q.shift();
    ...
    // [FASTER] Sliding Window Race
    // Instead of waiting for a batch, as soon as one probe fails, start the next.
        ...
        return new Promise((resolve, reject) => {
            ...
            const spawn = () => {
                ...
                probeSingle(url).then(validUrl => {
                    ...
                    active--;
                    failures++;
                    spawn(); // Replace this worker immediately
                });
            };

            // Start initial wave (max concurrent or total candidates)
            const waveSize = Math.min(6, candidates.length); // Per-image parallelism limit
    ...
    function generateCandidates(parsed) {
        ...
        return cand.sort((a,b) => a.p - b.p).filter(c => !unique.has(c.url) && unique.add(c.url)).map(c => c.url).slice(0, CONFIG.MAX_ATTEMPTS);
    }
    ...
    function applyFix(img, url) {
        ...
        brokenImageRegistry.forEach(otherImg => {
            ...
            // Just force the src update, let browser manage the queue
    ...
    function preemptiveFix(img) {
        ...
        const servers = Array.from(knownGoodServers);
        const sig = servers[servers.length - 1]; // Freshest
        const [p, n, r, t] = sig.split('|');
        ...
        // [FASTER] If preemptive fix hangs for > 1.5s, trigger full fix in background
        const slowTimer = setTimeout(() => {
             if (img.dataset.batoPreemptive === 'true' && !img.complete) {
                 addToQueue(img, 'high'); // Don't cancel preemptive, just race against it
             }
        }, 1500);
        ...
        img.addEventListener('load', () => {
            ...
            if (img.naturalWidth > 0) {
                ...
                img.dispatchEvent(new Event('error')); // Handle broken image
            }
        }, { once: true });
